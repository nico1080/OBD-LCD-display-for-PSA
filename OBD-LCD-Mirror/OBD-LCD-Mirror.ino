...
//To change screen you need to press the mirror left/right arrow on door
//At startup it display the init (0) screen for 10s (typre pressure oil level etc)
//Then it display the temprature (1) screen
//If you do a short press left/right arrow it will switch to screen (2) (air flow) then 3 (battery) then 4 (power) and go back to 0 (init)
//
//I also implemented "screen sequence " but I don't realy use it
//if you press the mirror up/down arrow on door you will go to screen 100 (sequence 0-->1) a left/right arrow will make you change screen in this sequence 100-->101-->100
...
unsigned long timerFPSCan;
unsigned long timerFPSDisplay;
...
//bool LastESCbutton;
unsigned long timerVCI;
//unsigned long timerESCbutton;
bool ChangeScreenLeft = false;
bool ChangeScreenRight = false;
bool ChangeSeqUp = false;
bool ChangeSeqDown = false;
...
// UDS CAN request is about 10ms or so. Debug parameter is optional : WaterTemperature = CanRequest(0x6A8, 0x688, 0xd4, 0x0A, 2, true); or WaterTemperature = CanRequest(0x6A8, 0x688, 0xd4, 0x0A, 2);
int CanRequest(int TxID, int  RxID, int adress1, int adress2, int Lenght, bool debug = false) {
  int result;
  unsigned long timercan;
  timercan = millis();
...
//KWP CAN request for change screen (rearview mirror switch)  request take 95/120ms with closing session& confirmation,   45/100ms with closing diag session witout confirmation   and 50ms without closing diag session
int RVSRequest( bool debug = false) {
  int result = 0x7F;
  unsigned long timercan = millis();
  //unsigned long timercan2=millis();


  //open diag session
  canMsgSnd.can_id = 0x74b; canMsgSnd.can_dlc = 0x03 ;  canMsgSnd.data[0] = 0x02;  canMsgSnd.data[1] = 0x10;  canMsgSnd.data[2] = 0xC0;
  CAN0.sendMessage( & canMsgSnd);
  if (debug) {
    Serial.println(" Open diag asked (10C0)");
  }


  timercan = millis();//init timer before loop
  do { //loop for receiving confirmation of diag session opened
    if (CAN0.readMessage( & canMsgRcv) == MCP2515::ERROR_OK) {
      int id = canMsgRcv.can_id;
      int len = canMsgRcv.can_dlc;
      //              if (debug) {
      ...
      //          Serial.println("");
      //        }
      if (id == 0x64B && len == 3 && canMsgRcv.data[0] == 0x02 && canMsgRcv.data[1] == 0x50 && canMsgRcv.data[2] == 0xC0) {
        if (debug) {
          Serial.print("diag opened confirmation (50C0)");
          Serial.print(" timer in "); Serial.println((millis() - timercan));
        }
        break; //we receive the confirmation so we break the do/while
      }
    }
  }
  while (millis() - timercan < 50);  //50ms CAN timeout for diag opening take 20ms
  delay(15);

  //ask id C0
  canMsgSnd.can_id = 0x74b; canMsgSnd.can_dlc = 0x03 ; canMsgSnd.data[0] = 0x02; canMsgSnd.data[1] = 0x21; canMsgSnd.data[2] = 0xC0;
  CAN0.sendMessage( & canMsgSnd);
  if (debug) {
    Serial.println(" ID 0xC0 asked  (21)");
  }

  timercan = millis();//init timer before loop
  do { //loop for receiving answer of C0 request  only first frame (10) ACK frame (30) is not send; so consecutive frame (21,22 etc) will not be received/read
    if (CAN0.readMessage( & canMsgRcv) == MCP2515::ERROR_OK) {
      int id = canMsgRcv.can_id;
      int len = canMsgRcv.can_dlc;
      if (id == 0x64b && len == 8 && canMsgRcv.data[0] == 0x10 && canMsgRcv.data[1] == 0x0E && canMsgRcv.data[2] == 0x61 && canMsgRcv.data[3] == 0xC0) {
        result = canMsgRcv.data[4]; //store value
        if (debug) {
          Serial.print("61: Data-4 received value is "); Serial.print((result), HEX);
          Serial.print(" timer in "); Serial.println((millis() - timercan));
        }
        break; //we receive the confirmation so we break the do/while
      }
  ...
  while (millis() - timercan < 200);  //200ms CAN timeout for receiving answer take between 20/100mm

  //  //ask diag session close
  //  canMsgSnd.can_id = 0x74b; canMsgSnd.can_dlc = 0x03 ; canMsgSnd.data[0] = 0x02; canMsgSnd.data[1] = 0x10; canMsgSnd.data[2] = 0x81;
  //  CAN0.sendMessage( & canMsgSnd);
  //  if (debug) {Serial.println(" closing diag asked (1081)");}

  //  timercan = millis();//init timer before loop
  //  do { //loop for receiving confirmation of diag session closed
  //    if (CAN0.readMessage( & canMsgRcv) == MCP2515::ERROR_OK) {
  //      int id = canMsgRcv.can_id;
  //      int len = canMsgRcv.can_dlc;
  //      if (id == 0x64b && len == 3 && canMsgRcv.data[0] == 0x02 && canMsgRcv.data[1] == 0x50 && canMsgRcv.data[2] == 0x81) {
  //                    if (debug) {Serial.println("diag closed confirmation (5081)");}
  //                    break; //we receive the confirmation so we break the do/while
  //        }
  //      }
  //    }
  //   while (millis() - timercan < 50);  //50ms CAN timeout for diag closing

  //if (debug) {Serial.print("Mirror check completed in:  ");Serial.println((millis()-timercan2));}
  return result;  //return value after confirmation that diag is closed
...
void DisplayFPS (int ScreenSelect, unsigned long timerFPSCan, unsigned long timerScreen) {
  ...
  unsigned long timerFPSDisplay = millis() - timerScreen;
...
void loop() {
  ...
  //Init screen
  if (millis() < 10 * 1000) { //Put init screen for 5sec
  ...
  if ( (millis() - timerVCI > 250)) {    //read VCI state every XXXms  (Lin request take about 80ms) & put in in ESCbutton
    timerVCI = millis();
    //unsigned long timercan2=millis();

    mirror = RVSRequest(0); //read mirror button selector
    //Serial.print("Mirror check completed in:  ");Serial.println((millis()-timercan2));

//        mirror=0x1;
//        Serial.print("mirror: ");Serial.println((mirror), HEX);
//        Serial.print("mirror BIN: ");Serial.println((mirror), BIN);

    if (((mirror  != 0x7F)) &&((mirror & 0x1) == 0x1)) { //last bit is 1 mean center position  is selected and we can read button to check if we need to change seq/screen  //Valid value received  (0x7F is invalid value)
        center = true;
        Last_left = left;
        Last_right = right;
        Last_up = up;
        Last_down = down;
        left = (bitRead(mirror , 6 ) == 1 );
        right = (bitRead(mirror , 7 ) == 1 );
        up = (bitRead(mirror , 5 ) == 1 );
        down = (bitRead(mirror , 4 ) == 1 );

        if ( !Last_left && (left != Last_left)) {
          ChangeScreenLeft = true;
          Serial.println(" change screen left");
        }// if was pushed & not pushed anymore)
        if ( !Last_right && (right != Last_right)) {
          ChangeScreenRight = true;
          Serial.println(" change screen right");
          }// if was pushed & not pushed anymore)
        if ( !Last_up && (up != Last_up)) {
          ChangeSeqUp = true;
          Serial.println(" change seq up");}// if was pushed & not pushed anymore)
        if ( !Last_down && (down != Last_down)) {
          ChangeSeqDown = true;
          Serial.println(" change seq down");
          }// if was pushed & not pushed anymore)
      }
      else {center = false;}
      //Serial.print("mirror &0x1: ");Serial.println((mirror & 0x1),BIN);
...
//      Serial.print(" down: "); Serial.println(down);
    ...
    if (ChangeSeqUp ||ChangeSeqDown ||ChangeScreenLeft ||ChangeScreenRight ) { // screen or sequence need to be change
      LastScreenSelect = ScreenSelect; //store last screen before checking screen
      ClearScreen = true;
      ...
      if (ChangeSeqUp) {
        switch (SeqSelect) {  //select new sequence
          case 2:          //last sequence
            SeqSelect = 0; break;
          default:         //-->increment
            SeqSelect = SeqSelect + 1; break;
        }
        //select start screen for each seqence
        if (SeqSelect == 0) {ScreenSelect = 1;}
        if (SeqSelect == 1) {ScreenSelect = 100;}
        if (SeqSelect == 2) {ScreenSelect = 200;}
        ChangeSeqUp = false;  //say we changed sequence
        ChangeScreenRight=false; //say we changed screen
        ChangeScreenLeft=false; //say we changed screen
      }

      if (ChangeSeqDown) {
        switch (SeqSelect) {  //select new sequence
          case 0:          //last sequence
            SeqSelect = 2; break;
          default:         //-->increment
            SeqSelect = SeqSelect - 1; break;
        }
        ...
        if (SeqSelect == 1) {ScreenSelect = 100;}
        if (SeqSelect == 2) {ScreenSelect = 200;}
        ChangeSeqDown = false;  //say we changed sequence
        ChangeScreenRight=false; //say we changed screen
        ChangeScreenLeft=false; //say we changed screen
      }

      if (ChangeScreenRight) {  //we need to change screen inside the sequence
        if (SeqSelect == 0) {
          switch (ScreenSelect) {  //switch stalement for sequence 0
            case 0:               //if init screen (0) go to screen x
              ScreenSelect = 1; break;
            case 4:               //if last screen (3) go to seq 0 (loop)
              ScreenSelect = 0; break; // return to xx screen
            default:         //for other screen -->increment
              ScreenSelect = ScreenSelect + 1; break;
          }
        }
      ...
      if (ChangeScreenLeft) {  //we need to change screen inside the sequence
        if (SeqSelect == 0) {
          switch (ScreenSelect) {  //switch stalement for sequence 0
            case 0:               //if init screen (0) go to screen x
              ScreenSelect = 4; break;
            default:         //for other screen -->increment
              ScreenSelect = ScreenSelect - 1; break;
          }
        }
        ...
        if (SeqSelect == 1) {
          switch (ScreenSelect) {  //switch stalement for sequence 0
            case 100:               //if last screen (3) go to seq 0 (loop)
              ScreenSelect = 101; // return to xx screen
              break;
            default:         //for other screen -->increment
              ScreenSelect = ScreenSelect - 1;
              break;
        ...
        if (SeqSelect == 2) {
          switch (ScreenSelect) {  //switch stalement for sequence 0
            case 200:               //if last screen (3) go to seq 0 (loop)
              ScreenSelect = 201; // return to xx screen
              break;
            default:         //for other screen -->increment
              ScreenSelect = ScreenSelect - 1;
              break;
